...
async function openTicket(interaction, categoryId, subject = null) {
  ...
  const embed = new EmbedBuilder()
    .setColor(parseInt((category.color || '#5865F2').replace('#', ''), 16))
  ...
  if (config.log_channel) {
    guild.channels.cache.get(config.log_channel)?.send({
      embeds: [new EmbedBuilder()
        .setColor(0x57F287)
        .setTitle('🎫 Ticket Abierto')
        .addFields(
...
async function closeTicket(interaction, ticketId, reason = null) {
  ...
  // Prioridad: Supabase → SQLite (fallback)
  let transcriptChannelId = config.transcript_channel; // valor por defecto (SQLite)
  try {
    const cloudConfig = await getGuildConfig(guild.id);
    if (cloudConfig?.transcript_channel_id) {
      transcriptChannelId = cloudConfig.transcript_channel_id; // Supabase gana si existe
    }
  } catch (err) {
    console.warn('⚠️ No se pudo leer config de Supabase, usando SQLite:', err.message);
  }

  if (transcriptChannelId) {
    const tchan = guild.channels.cache.get(transcriptChannelId);
    if (tchan) {
      const att = new AttachmentBuilder(filepath, { name: filename });
      tchan.send({
        embeds: [new EmbedBuilder()
          .setColor(0x5865F2)
  ...
  if (config.dm_on_close) {
    try {
      ...
      if (creator) {
        const att2 = new AttachmentBuilder(filepath, { name: filename });
        await creator.send({
          embeds: [new EmbedBuilder()
            .setColor(0xED4245)
  ...
  // ── LOG GENERAL ───────────────────────────────────────────────────────────
  if (config.log_channel) {
    const att3 = new AttachmentBuilder(filepath, { name: filename });
    guild.channels.cache.get(config.log_channel)?.send({
      embeds: [new EmbedBuilder()
        .setColor(0xED4245)
        .setTitle('🔒 Ticket Cerrado')
